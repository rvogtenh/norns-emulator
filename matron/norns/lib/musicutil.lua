...
function MusicUtil.snap_note_to_array(note_num, snap_array)
  if not note_num or not snap_array or #snap_array == 0 then return note_num end
...
function MusicUtil.note_num_to_freq(note_num)
  -- nil guard (mirrors note_num_to_name): scripts like awake index a scale
  -- array with a computed index that can run past its end → note_num == nil.
  -- Returning nil yields a rest instead of crashing the clock thread.
